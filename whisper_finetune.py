...
from common import visualise_dataloader
...
class WhisperFineTune:
    ...
    def __init__(self, config: dict, train_data: dict):
        ...
        self.unfreeze_layers = int(config["embeddings_config"].get("unfreeze_layers",0))
    ...
    def define_optimizer(self, model):
        ...
        lr_scheduler = ExponentialLR(optimizer, gamma=self.config["decay_rate"])
    ...
    def compile_model_freezed(self):
        ...
        self.model = WhisperForClassification(model_name=f'openai/{self.config["embeddings_config"]["model_extractor"]}', 
                                              n_out=self.config["n_out"])
        ### whisper.load_model(self.config["embeddings_config"]["model_extractor"].split('-')[1]).encoder # small, base, tiny
        # Set to GPU/CPU:
        self.model.to(self.device)
        
        # Define loss function and optimizer for the classifier only:
        self.criterion = CrossEntropyLoss() if self.config["n_out"] > 1 else BCELoss()
    ...
    def compile_unfreeze_encoder(self, num_layers=3):
        ...
        if self.unfreeze_layers > 0:
            ...
            if self.config["embeddings_config"].get("unfreeze_first_last", "last" ) == "first":
    ...
    def _train(self, train_loader, epochs, verbose=2):
        ...
        early_stopper = EarlyStopper(patience=self.config["early_stop"]["patience"]) if \
            self.config['early_stop'].get('evaluate', True) else None
        ...
        for epoch in range(epochs):
            ...
            for i_batch, (x_batch, y_batch) in enumerate(train_loader):
                ...
                # Run the input throught the model
                ...
                if not (i_batch+1)%verbose:
                    print(f"**Epoch #{epoch+1}/{epochs}: Batch #{i_batch+1}/{len(train_loader)}, Loss: {loss/len(x_batch):.4f}, acc = {100*acc_batch/y_batch.size(0):.2f}%")
            ...
            self.scheduler.step()
            print(f"Done Epoch #{epoch+1}/{epochs} ({time_epoch/60:.2f}min): Loss: {epoch_loss/len(train_loader):.4f}, Accuracy = {accuracy:.2f}%")
    ...
    def visualize_data_classes(self, sampler):
        ...
        dl = DataLoader(ds, batch_size=int(self.config["batch_size"]),
                        worker_init_fn=seed_worker, generator=g)
        
        fig_no_balan = visualise_dataloader(dl)
        dl = DataLoader(ds, batch_size=int(self.config["batch_size"]), 
                        sampler=sampler, worker_init_fn=seed_worker, generator=g)
        fig_with_balan = visualise_dataloader(dl)
